...
saveDir    = 'C:\Users\STB\Desktop\Araştırma\Model2_Early_RandomSplit';
...
fprintf('\n🚀 MODEL 2: 2D-CNN + 1D-CNN EARLY FUSION BAŞLIYOR (RASTGELE BÖLME)...\n\n');
...
%% 4. MİMARİ: EARLY FUSION (2D-Visual & 1D-Audio)
lgraph = layerGraph();
...
% --- Erken Füzyon (Early Fusion) Birleştirme Masası ---
shared = [
...
lgraph = connectLayers(lgraph, 'a_flat', 'early_concat/in2');
...
title('Training Progress (2D + 1D Early Fusion)'); xlabel('Epochs'); ylabel('Accuracy (%)'); legend('Train', 'Test'); grid on;
...
function [YPred, YTrue, accTotal, accVis, accAud] = evaluateModel(net, mbq, calcSingle)
    ...
    while hasdata(mbq)
        ...
        % 2. Eksik Veri Simülasyonu (Single Modality Test)
        if calcSingle
            % Sesi tamamen sıfırlarla doldurup test et
            outV = predict(net, XV, zeros(size(XA),'single')); 
            [~, vIdx] = max(extractdata(outV),[],1);
            YP_Vis=[YP_Vis, vIdx];
            
            % Görüntüyü tamamen sıfırlarla doldurup test et
